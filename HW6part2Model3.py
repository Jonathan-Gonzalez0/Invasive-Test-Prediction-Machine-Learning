...
Parte 2: MODELO #3
Modelo 3 de 'machine learning' con una implementacion de 'gradient descent logistic regression' que 
...
w1,w2,w3,w4,w5,b = 0,0,0,0,0,0                     # initial values for w1,w2 and b
...
x1x2exp = np.exp(x1*x2)

z = w1*x1 + w2*x2 + w3*x1e + w4*x2sqr + w5*x1x2exp + b # Equation of model
...
for k in range(1,niter):
    ...
    w5     = w5 - alpha/m*np.sum((ys-y)*(x1x2exp))    # update w5
    b      = b  - alpha/m*np.sum((ys-y)*(1))     # update b
    z = w1*x1 + w2*x2 + w3*x1e + + w4*x2sqr + w5*x1x2exp + b
    ys     = 1/(1+np.exp(-z)) 
...
modelo = lambda x,y: w1*(x) +w2*(y) + w3*(np.exp(x)) + w4*(y**2) + w5*(np.exp(x*y)) +b
